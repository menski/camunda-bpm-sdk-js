...
for (id in _store.processDefinition) {
  _store.processDefinitionFormVariables[id] = {
    ...
    stringVar: {
      ...
      name: 'stringVar',
      type: 'string',
      value: fixturer.randomString(4, 16),
      processInstanceId: id,
      executionId: 'b68b71c9-e310-11e2-beb0-f0def1557726',
      taskId: null,
      activityInstanceId: 'Task_1:b68b71ca-e310-11e2-beb0-f0def1557726'
...
HttpClient.prototype.get = function(path, options) {
  ...
  switch (resourceName) {
    case 'process-definition':
      if (pathParts[pathParts.length - 1] === 'form-variables') {
        results = _store.processDefinitionFormVariables[pathParts[pathParts.length - 2]];
      }
      else {
